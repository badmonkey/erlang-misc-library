...
usage(OptSpecList, ProgramName, OutputStream) when is_atom(OutputStream) ->
    F = formatter(OutputStream),
    F( "~ts~n~n~ts~n"
     , [ unicode:characters_to_list(usage_cmd_line(ProgramName, OptSpecList))
       , unicode:characters_to_list(usage_options(OptSpecList))
       ]);
...
usage(OptSpecList, ProgramName, CmdLineTail, OutputStream) when is_atom(OutputStream) ->
    F = formatter(OutputStream),
    F( "~ts~n~n~ts~n"
     , [ unicode:characters_to_list(usage_cmd_line(ProgramName, OptSpecList, CmdLineTail))
       , unicode:characters_to_list(usage_options(OptSpecList))
       ]);
...
usage(OptSpecList, ProgramName, CmdLineTail, OptionsTail, OutputStream) ->
    ...
    F( "~ts~n~n~ts~n"
     , [ unicode:characters_to_list(usage_cmd_line(ProgramName, OptSpecList, CmdLineTail))
       , unicode:characters_to_list(usage_options(OptSpecList, OptionsTail))
       ]).


formatter(string) -> fun(M, A) -> io_lib:format(M, A) end;
formatter(IoD) -> fun(M, A) -> io:format(IoD, M, A) end.
