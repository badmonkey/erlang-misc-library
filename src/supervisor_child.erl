...
-spec build_specs( [atom() | tuple()], build_type() ) -> [supervisor:child_spec()].
...
get_spec({Id, Mod, Args, Restart, SpecType}, _Type) ->
    ?GENERIC_SPEC(Id, Mod, Args, Restart, SpecType);
...
get_spec(Id, Module, Args, Type)
    ...
    ;   {false, supervisor, _}  -> ?SUPERVISOR_SPEC(Id, Module, Args)
    ;   {false, gen_server, _}  -> ?SERVICE_SPEC(Id, Module, Args)
    ;   {false, _, _}           -> ?SERVICE_SPEC(Id, Module, Args)
    end.
