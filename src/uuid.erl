...
-export([version/1]).
-export([new_v1/0, new_v1/2, new_v3/2, new_v4/0, new_v4/4, new_v4_snowflake/0, new_v5/2]).
...
-type uuid_rec() :: {uuid, uuid_version(), uuid_binary()}.
-type uuid_type() :: uuid_binary() | string() | uuid_rec().
...
-spec version_field( uuid_binary() ) -> pos_integer().
...
new_v1() -> new_v1(undefined, undefined).


-spec new_v1( <<_:48>> | undefined, <<_:14>> | undefined ) -> {uuid, 1, uuid_binary()}.

new_v1(undefined, undefined)                    -> new_v1_compose( get_node_id(), get_clock() );
new_v1(undefined, <<_:14>> = ClockSeq)          -> new_v1_compose( get_node_id(), ClockSeq );
new_v1(<<_:48>> = Node, undefined)              -> new_v1_compose( Node, get_clock() );
new_v1(<<_:48>> = Node, <<_:14>> = ClockSeq)    -> new_v1_compose( Node, ClockSeq ).

new_v1_compose(<<Node:48>>, <<ClockSeq:14>>) ->
    <<TimeHi:12, TimeMid:16, TimeLow:32>> = xtime:gregorian_epoch(),
    <<ClockSeqHi:6, ClockSeqLow:8>> = <<ClockSeq:14>>,
...
-spec new_v3( atom() | uuid_type(), string() ) -> {uuid, 3, uuid_binary()}.
...
-spec new_v4(<<_:48>>, <<_:12>>, <<_:14>>, <<_:48>>) -> {uuid, 4, uuid_binary()}.
...
new_v4_snowflake() ->   
    {ok, <<Time:48, Time2:12, Time3:4, WorkerId:10, WorkerId2:38, _Drop:6, Sequence:10>>} = snowflake:get_id(),
    {uuid, 4, <<Time:48, ?UUIDv4:4, Time2:12, ?VARIANT10:2, Time3:4, WorkerId:10, WorkerId2:38, Sequence:10>>}.
...
-spec new_v5( atom() | uuid_type(), string() ) -> {uuid, 5, uuid_binary()}.
...
-spec to_binary( string() ) -> uuid_binary().
...
-spec create_namebased_uuid(md5 | sha1, uuid_binary()) -> {uuid, 3 | 5, uuid_binary()}.
...
-spec compose_namebased_uuid(3 | 5, uuid_binary()) -> {uuid, 3 | 5, uuid_binary()}.
