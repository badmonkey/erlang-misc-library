...
{DEC256}{DOT}{DEC256}{DOT}{DEC256}{DOT}{DEC256} : ?TOKEN(ipv4_address, TokenLine, TokenChars).
{ATOM}                                          : ?TOKEN(atom, TokenLine, TokenChars).
{VAR_BEGIN}{ATOM}({DOT}{ATOM})*{VAR_END}        : ?TOKEN(variable, TokenLine, strip_var(TokenChars)).
{SPECIAL}                                       : ?FORWARD_TOKEN(TokenLine, TokenChars).

{QUOTE}{QUOTE}                                  : ?TOKEN(substring, TokenLine, "").
{QUOTE}{VAR_BEGIN}{ATOM}({DOT}{ATOM})*{VAR_END} : ?TOKEN_PUSH(substitute, TokenLine, "\"", strip_var(strip_junk(TokenChars))).
{QUOTE}{VAR_BEGIN}                              : ?TOKEN_PUSH(strstring, TokenLine, "\"", "${").
...
-define(FORWARD_TOKEN(Line, Text), make_token(hd(Text), Line, Text, token)).
