...
start_link(CallbackModule, Socket, InitParams)
    ...
    start_link(CallbackModule, ?MODULE, Socket, InitParams).
    

start_link(CallbackModule, Name, Socket, InitParams)
        when is_atom(CallbackModule), is_list(InitParams) ->
    gen_server:start_link(Name, ?MODULE, [CallbackModule, Socket, InitParams], []).
...
init([CallbackModule, Socket, InitParams]) ->
    ...
    try
        case CallbackModule:init(Socket, NInitParams) of
...
handle_info( {tcp, Socket, Data}
    ...
    try
        ...
        case CallbackModule:handle_data({raw, FullData}, Socket, ProxyState) of
            {ok, NewState}                          ->
    ...
    try
        ...
        case get_packet(Mode, FullData) of
            {ok, Value, Rest} when is_binary(Rest) ->
                case CallbackModule:handle_data({packet, FullData}, Socket, ProxyState) of
                    {ok, NewState}                          ->
...
handle_info( {tcp_closed, Socket}
    ...
    try
        case CallbackModule:handle_data({closed, Buffer}, Socket, ProxyState) of
            {stop, Reason, NewState}    ->
...
handle_info( {tcp_error, Socket, Reason}
           , #state{module = CallbackModule, proxystate = ProxyState, buffer = Buffer, socket = Socket} = State) ->
    try
        case CallbackModule:handle_data({error, Reason, Buffer}, Socket, ProxyState) of
            {reply, Reply, NewState}    ->
                gen_tcp:send(Socket, Reply),
                {stop, {tcp_error, Reason}, State#state{proxystate = NewState}}
...
validate_mode(F) when is_function(F, 1) -> true;
...
get_packet(F, Bytes) when is_function(F, 1) -> F(Bytes);
